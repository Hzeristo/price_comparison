...
@Service
public class TaobaoSpiderService {

    private WebDriver driver;

    public TaobaoSpiderService() {
    ...
    public void loginAndSaveCookies(String loginUrl, String username, String password) throws IOException {
        ...
        driver.findElement(By.cssSelector("button.fm-submit.password-login")).click();

        // 等待页面跳转或登录成功标识元素出现
        new WebDriverWait(driver, Duration.ofSeconds(60)).until(ExpectedConditions.presenceOfElementLocated(By.id("mtb-app")));
    ...
    private void saveCookies() throws IOException {
        File file = new File("src/main/resources/taobaocookies.data");
        if (!file.exists()) {
    ...
    public List<Map<String, String>> fetchProducts(String searchUrl) {
        ...
        try {
            ...
            // 等待商品列表加载完成
            new WebDriverWait(driver, Duration.ofSeconds(30)).until(
                ExpectedConditions.presenceOfElementLocated(By.id("content_items_wrapper"))
            );
            System.out.println("商品列表加载完成！");
            // 获取商品列表
            List<WebElement> items = driver.findElements(By.cssSelector(".tbpc-col.search-content-col"));
            ...
            for (WebElement item : items) {
                try {
                    WebElement mainUrlElement = item.findElement(By.cssSelector("a"));
                    String mainUrl = mainUrlElement.getAttribute("href");
                    System.out.println("主要信息url: " + mainUrl);
                    ...
                    if (matcher.find()) {
                        skuid = matcher.group(1); // 获取匹配的第一个分组
                        System.out.println("提取到的 skuId: " + skuid);
                    } else {
                        skuid = "0";
                        System.out.println("未找到 skuId");
                    }

                    WebElement nameElement = item.findElement(By.cssSelector("a div div div[class*='descWrapper'] div span"));
                    String name = nameElement.getText();
                    System.out.println("商品名称：" + name);

                    //button
                    System.out.println("商品链接：" + mainUrl);

                    WebElement imgElement = item.findElement(By.cssSelector("a div div div[class*='mainPicAdaptWrapper'] img[class*='mainPic']"));
                    String image = imgElement.getAttribute("src");
                    System.out.println("商品图片：" + image);

                    // 提取价格
                    String price = item.findElement(By.cssSelector("a div div div[class*='priceWrapper'] div")).getText();
                    ...
                    product.put("url", mainUrl);
                    product.put("image", image);
                    product.put("price", price);
                    product.put("platform", "淘宝");
    ...
    public Map<String, String> fetchProduct(String searchUrl) {
        ...
        try {
            ...
            // 等待商品列表加载完成
            new WebDriverWait(driver, Duration.ofSeconds(30)).until(
                ExpectedConditions.presenceOfElementLocated(By.cssSelector(".tbpc-col.search-content-col"))
            );
            System.out.println("商品列表加载完成！");
            // 获取商品列表
            List<WebElement> items = driver.findElements(By.cssSelector(".tbpc-col.search-content-col"));
